...
class Controller(ABC):
    ...
    def _get_pi_compatible_reagent_payload(self):
        '''
        Returns a detached copy of the measured reagent data for the Pi.

        The Raspberry Pi owns the calibrated tube tare constants. Sending
        measured tube-plus-solution masses unchanged ensures its established
        ``mass - tare`` calculation derives the correct liquid volume while
        preserving controller audit records.
        '''
                ...
                if "water" in str(full[full['time']==latest_transfer_time]['chem'].item()).lower():
                    ...
                    while stillWater:
                        if len(transfers_before_scans)>1:
                            print(f"remove this {latest_transfer_time}")
                            print(f"list before remove {transfers_before_scans}")
                            transfers_before_scans.remove(latest_transfer_time)
                            print(f"list after remove {transfers_before_scans}")
                            latest_transfer_time = max(transfers_before_scans)
