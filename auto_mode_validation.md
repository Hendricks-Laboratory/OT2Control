...
| Stage 11A retired remote-publication boundary | Offline test-only historical code; not integrated | `auto_live_run_remote.py` is a dependency-free contract that was never connected to the controller. It does not load a cloud SDK or credentials, make a network call, change the Pi, alter the original workbook, or operate on remote files. It is retained as inert historical test coverage and is not the planned Drive publication mechanism. |
...
### Stage 12F-A target-then-stability configuration contract — 2026-09-16

Stage 12F-A recognizes `auto_stability_mode=target_then_stability` without
silently treating it as monitoring. It requires the ordinary enabled-stability
Header settings plus an explicit positive finite
`auto_stability_replicate_log10_loss_rate_sd_max`, expressed in
`log10(absorbance loss / second)` units. This is the future condition-level
replicate-variability ceiling; it has no default because its scientific
strictness must be chosen deliberately.

The controller validates the final trigger-reagent order and requires the
single legacy `exploit` acquisition interface, no portfolio, and at least
three physical duplicates. It then intentionally raises a clear pre-hardware
validation error. Thus Stage 12F-A changes no scan, shake, robot, Pi,
primary-GP, stability-GP, or recipe-ranking behavior. `stability_only`,
pipette mixing, no-added mixing, `target_ei`, and `core3` remain unavailable
for active stability selection until their dedicated design and validation.

### Stage 12F-B target-then-stability selection — 2026-09-17

Stage 12F-B enables the explicitly configured
`auto_stability_mode=target_then_stability` pathway on the Lab PC only. It
does not change Auto-main/Pi code, reader command syntax, plate-shake policy,
ordinary `off`/`monitor` Auto behavior, primary wavelength-GP history, target
stopping, source-volume preflight, or controller-side recipe validation.

- The existing primary wavelength GP remains the first lexicographic gate.
  The optimizer searches only candidates that retain all ordinary mask,
  exact-zero, executable-transfer, water-top-off, overflow, and controller
  volume rules. Where the trigger reagent is a variable, its OFF masks are
  excluded before optimization rather than repaired after selection.
- When the companion stability GP has fewer than two accepted conditions, the
  run is explicitly labeled `lambda_bootstrap_stability_model_insufficient`
  and uses the existing target-distance (`exploit`) objective with the trigger
  mask still ON. A companion reporting/fit failure instead blocks the next
  proposal before new wells, reader work, or liquid handling.
- Once fitted, the companion GP predicts
  `log10(loss_rate_absorbance_per_s)` independently. It ranks only numerically
  found candidates whose primary-GP mean lies within the existing Header
  wavelength tolerance. The lower predicted log loss rate wins; no arbitrary
  wavelength/stability weighted sum is introduced. If that constrained search
  finds no target-compatible physically executable result, it explicitly falls
  back to the wavelength-target objective rather than selecting a stable blank.
- Active stability training additionally requires a finite replicate sample SD
  on the log10 loss-rate scale and rejects a complete condition that exceeds
  the explicit Header limit
  `auto_stability_replicate_log10_loss_rate_sd_max`. Raw loss-rate SD and all
  rejected provenance remain in the stability audit CSV.
- Selection-time stability status, companion model state, trigger-mask policy,
  target compatibility tolerance/count, predicted log loss-rate mean/SD, and
  raw-unit loss-rate conversion are retained in the condition performance log
  and recipe-design provenance. The final report distinguishes passive
  monitoring from active lexicographic selection.

Hardware-free validation on 2026-09-17 used Python 3.9.6. Compilation,
AST/duplicate-method checks, focused synthetic target-first/stability-second
tests, mask/trigger/fallback tests, model-QC tests, controller source-contract
tests, and the complete 375-test suite passed. The full suite includes legacy
mask-selection, controller handoff, report, volume, import, and checkpoint
regression coverage. This is not physical clearance: the required next gate
is the third human-controlled dry debug with `target_then_stability` enabled
and a safe small batch before any chemistry run.

### Stage 12F-C standalone stability-only configuration contract — 2026-09-18

The scientific priority has been refined: standalone optical-stability
selection precedes the optional target-then-stability hybrid. Stage 12F-C now
accepts `auto_stability_mode=stability_only` as a deliberately **target-free**
Header contract on the Lab PC.

- `target` and `target_tolerance_nm` must both be blank or omitted. This is
  checked in the shared Header parser before Auto-specific configuration, so a
  blank spreadsheet cell represented internally as `NaN` is handled as blank
  rather than producing an accidental numeric wavelength objective.
...
- The current protocol validates trigger order, plate-shake cadence, the
  ...
  wavelength target, target stopping, or lambda-selection gate is configured.
...
preflight, or existing `target_then_stability` behavior. Hardware-free
validation used Python 3.9.6: compilation, focused Header/controller tests,
and the complete 378-test suite passed. It is configuration validation only;
it requires no dry debug and is not physical clearance.

### Stage 12F-D reference-peak signal companion model — 2026-09-18

Stage 12F-D adds the independent optical-signal evidence needed before a
future stability-only selector can reject blank, weak, or saturated candidates
without reintroducing a hidden wavelength target.

- Stage-12D condition summaries now separately aggregate finite
  fixed-reference peak absorbance across the planned physical replicate set.
  `condition_signal_status` is `complete`, `partial`, or
  `no_eligible_signal_wells`; it does not alter the established
  `condition_stability_status` or loss-rate QC.
- A finite reference peak is retained as signal evidence even if its trajectory
  was low signal or had no observed post-peak decline. Low and high signal
  observations are therefore not silently discarded before later candidate
  bounds are applied.
- `auto_stability_signal_model.py` provides a distinct cumulative GPy
  regression from normalized recipe to raw blank-corrected reference-peak
  absorbance. It has its own state, units, audit fields, and atomic
  no-history-regression guard; it cannot mutate the lambda-max GP or the
  Stage-12E loss-rate GP.
- Training requires complete current-run manifest provenance and an exact
  recipe mapping. Imported lambda-only history, missing/partial evidence,
  duplicate identities, orphan summaries, and out-of-bounds recipes receive
  explicit rejected audit records. Signal bounds are deliberately not training
  exclusions: they are future user-selected candidate constraints.

This is still a pure model/reporting layer. It adds no controller model
refresh, candidate selection, simulation, reader/robot command, Auto-main/Pi
change, target-stopping change, or normal Auto behavior change. Hardware-free
validation used Python 3.9.6: compilation, aggregation/reporting coverage,
...
logs, and plots without changing recipe selection. The first intended active
mode is `stability_only`: it has no wavelength target and will minimize the
...
| `auto_preparation_mode` | `off` (legacy default) or `required`. `required` requires a validated `auto_preparation` worksheet and runs its grouped working-solution preparation once after the Auto-main handshake and before seed generation. It cannot currently be combined with checkpoint import. The worksheet uses `enabled`, `stock_source_group`, `stock_concentration_mM`, `working_concentration_mM`, `tube_count`, `final_volume_per_tube_uL`, `destination_labware`, and `destination_container`; optional ordered `destination_locs` and `destination_deck_positions` reserve declared-`empty` locations, while optional `water_source_policy` accepts `auto` (location-based behavior), `temperature_controlled`, or `ambient`. `temperature_controlled` selects water on the temperature module; its actual temperature comes from Header settings. Each enabled row produces identical working tubes through water → stock → mix. Auto-main remains the authority for physical destination availability and capacity. |
