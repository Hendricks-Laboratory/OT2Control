...
| Auto early terminal transcript | Implemented; dry debug pending | Auto buffers Header parsing, safety warnings, and output-directory collision messages in memory after the Header download. Once the approved per-run `Debug/terminal_output.txt` can be opened, it prepends that transcript and continues normal live capture. Early setup failure restores normal streams and creates no transcript artifact. Isolated stream/restoration, controller-placement, and collision-guard tests passed. |
