...
| Import-only run-context lineage and tabular history snapshots | Implemented, no cross-run plots yet | Existing-output imports flatten and checksum-identify ancestor runs without nesting; they export de-duplicated native condition and well-level replicate CSV snapshots plus source-availability diagnostics. Manual checkpoint imports remain model-only. Raw scan ingestion and cross-run plots remain deferred. Python 3.9 isolated lineage, branching, duplicate-conflict, legacy, and tabular-history tests passed. |
