...
| Stage 9B Auto-main group reservation and execution | Pending | The earlier one-destination execution path is no longer reachable through `auto_preparation_mode=required`. Until Auto-main can reserve grouped stock and destination tubes, select cold/standard water from the actual stock placement, and register multiple completed working tubes, a real run fails closed before seed generation. Local preflight remains planning-only. |
