...
| Stage 11A immutable remote-publication boundary | Implemented; hardware-free validation passed | `auto_live_run_remote.py` is an offline, dependency-free contract for a later remote adapter. From an already verified local queue item, it derives one content-addressed `Auto_Live_Runs/<run>/status/revision_<N>_<digest>.xlsx` destination and permits only `create_immutable_snapshot` with `overwrite: false`. It verifies the local immutable workbook checksum before invoking an injected fake publisher and rejects receipt tampering. No cloud SDK, credentials, network call, controller integration, Pi change, original-workbook modification, or remote file operation is present in this stage. |
